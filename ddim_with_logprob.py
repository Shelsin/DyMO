...
def ddim_step_with_logprob(
    ...
    # log prob of prev_sample given prev_sample_mean and std_dev_t  # standard distribution = compute x_t-1
    log_prob = (
    ...
    return prev_sample.type(sample.dtype), pred_original_sample, log_prob



def ddim_step_with_logprob_dymo(
    self: DDIMScheduler,
    model_output: torch.FloatTensor,
    timestep: int,
    sample: torch.FloatTensor,
...
def ddim_step_forward_from_xt_1_to_xt(
        self: DDIMScheduler,
        timestep: int,
        x_prev: torch.FloatTensor,
        generator=None,
) -> Union[DDIMSchedulerOutput, Tuple]:
    """
    Args:
    ...
    alpha_prod_t = _left_broadcast(alpha_prod_t, x_prev.shape)
    alpha_prod_t_prev = _left_broadcast(alpha_prod_t_prev, x_prev.shape)

    beta_t = alpha_prod_t/alpha_prod_t_prev

    variance_noise = randn_tensor(
        x_prev.shape, generator=generator, device=x_prev.device, dtype=x_prev.dtype
    )

    x = beta_t.sqrt() * x_prev + (1 - beta_t).sqrt() * variance_noise

    return x.type(x_prev.dtype)
